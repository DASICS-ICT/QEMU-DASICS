...
static int dasics_sreg_slot(uint32_t regno)
{
    switch (regno) {
    case 8:
        return 0;
...
static void dasics_sreg_guard_reset(dasics_table_t *state)
{
    int i;

    /* M4.Q1-alpha defaults: per-slot fixed-key PRF+MAC, 64-bit tag */
...
static inline void dasics_sreg_clear_slot(dasics_table_t *state, int slot)
{
    state->sreg.phase[slot] = SREG_PHASE_INIT_LOCKED;
    state->sreg.saved_once[slot] = 0;
    state->sreg.saved_addr[slot] = 0;
    state->sreg.sp_off[slot] = 0;
    state->sreg.shadow_cipher[slot] = 0;
    state->sreg.shadow_tag_lo[slot] = 0;
    state->sreg.shadow_tag_hi[slot] = 0;
}

static const char *dasics_sreg_slot_name(int slot)
{
    static const char *const names[DASICS_SREG_COUNT] = {
        "s0", "s1", "s2", "s3", "s4", "s5",
        "s6", "s7", "s8", "s9", "s10", "s11",
    };

    if (slot < 0 || slot >= DASICS_SREG_COUNT) {
        return "unknown";
    }
    return names[slot];
}

static void dasics_trace_sreg_auth_fault(CPURISCVState *env, target_ulong pc,
                                         uint32_t regno, int slot,
                                         target_ulong curr_value)
{
    target_ulong initial_plain = env->dasics_state.sreg.saved_addr[slot];
    target_ulong sealed_token = env->dasics_state.sreg.shadow_cipher[slot];

    fprintf(stderr,
            "[M4-TRACE] AUTH fault slot=%s reg=x%d pc=0x" TARGET_FMT_lx
            " initial_plain=0x" TARGET_FMT_lx
            " sealed_token=0x" TARGET_FMT_lx
            " tampered_current=0x" TARGET_FMT_lx
            " fault_sreg_value=0x" TARGET_FMT_lx "\n",
            dasics_sreg_slot_name(slot), regno, pc, initial_plain,
            sealed_token, curr_value, curr_value);
}

static void dasics_raise_sreg_fault(CPURISCVState *env, target_ulong bad,
                                    target_ulong reason)
{
...
target_ulong helper_dasics_sreg_read(CPURISCVState *env, target_ulong pc,
                                     uint32_t regno, target_ulong curr_value)
{
    ...
    trusted = dasics_in_trusted_zone(env, pc);
    phase = env->dasics_state.sreg.phase[slot];

    if (!trusted) {
        if (phase == SREG_PHASE_ACTIVE) {
            return curr_value;
        }

        dasics_sreg_crypto_seal_a(curr_value, regno, &cipher, &tag);
        env->gpr[regno] = cipher;
        env->dasics_state.sreg.saved_addr[slot] = curr_value;
        env->dasics_state.sreg.shadow_cipher[slot] = cipher;
        env->dasics_state.sreg.shadow_tag_lo[slot] = tag;
        env->dasics_state.sreg.shadow_tag_hi[slot] = 0;
    ...
    ret = dasics_sreg_crypto_open_a(curr_value, regno,
                                    env->dasics_state.sreg.shadow_tag_lo[slot],
                                    &cipher);
    if (ret != 0) {
        dasics_trace_sreg_auth_fault(env, pc, regno, slot, curr_value);
        dasics_raise_sreg_fault(env, slot, DFR_S0_AUTH);
    }

    env->gpr[regno] = cipher;
    dasics_sreg_clear_slot(&env->dasics_state, slot);
    return cipher;
}

void helper_dasics_sreg_prepare_write(CPURISCVState *env, target_ulong pc,
                                      uint32_t regno, target_ulong old_value)
{
    int slot = dasics_sreg_slot(regno);
    target_ulong cipher;
    target_ulong tag;
    ...
    dasics_sreg_crypto_seal_a(old_value, regno, &cipher, &tag);
    env->gpr[regno] = cipher;
    env->dasics_state.sreg.saved_addr[slot] = old_value;
    env->dasics_state.sreg.shadow_cipher[slot] = cipher;
    env->dasics_state.sreg.shadow_tag_lo[slot] = tag;
    env->dasics_state.sreg.shadow_tag_hi[slot] = 0;
    env->dasics_state.sreg.phase[slot] = SREG_PHASE_ACTIVE;
